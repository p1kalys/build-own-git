...
def _ls_tree(content: bytes) -> list[GitFile]:
    ...
    while True:
        ...
        files_sha, content = rest[:20], rest[20:]
        mode_raw, filename = file_header.decode().split()
        mode = int(mode_raw, 8)
        hex_digest = files_sha.hex()
...
def commit_tree():
    ...
    assert p == "-p"
    assert m == "-m"
    author = "Gibson Galeo <gibs@gmail.com>"
